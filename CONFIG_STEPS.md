...
## 三、导入覆写脚本（核心步骤-Carl整理版）

- 基于：https://github.com/IvanSolis1989/Smart-Config-Kit/tree/main
- 可根据自我情况修改

### Mihomo Party

1. 打开“内核”
2. 左侧菜单 → **覆写（Override）** → 右上角 ➕。
3. 类型选择 **JavaScript（.js）**。
4. 名称：`Clash Smart v5.2.2`， 打开“全局/Global”。
5. 内容：复制 `Clash Party/Clash Smart内核覆写脚本.js` 的**全部 2246 行**粘贴进去。
6. 保存。
7. 返回「订阅」页面，右键你的订阅 → **编辑信息/Edit Information** → 点击+添加**覆写/Override** → 勾选刚才的脚本 → 保存。
8. 切换到该订阅，点击「**连接**」。
...
如果你觉得每次打开终端都要敲这两行咒语太麻烦，我们可以把它写进你的 Mac 终端配置文件（`.zshrc`）里，做成一个“快捷开关”。

在终端里输入：

```
echo 'alias setproxy="export http_proxy=http://127.0.0.1:7890; export https_proxy=http://127.0.0.1:7890; echo \"✅ System Proxy On ✌🏻\""' >> ~/.zshrc
source ~/.zshrc
```
