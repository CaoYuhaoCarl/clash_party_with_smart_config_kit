...
// v5.2.1 变更摘要（5项修复，基于 04-01~04-09 日志分析）：
...
const REGION_DB = [
  ...
  { id: 'TW', kw: ['台湾', '台北', '台中', '高雄', '新北', '桃园', 'taiwan', 'taipei', 'taichung', 'kaohsiung', 'tpe'], iso: ['TW'] },
  { id: 'CN', kw: ['中国', '大陆', '国内', '中国大陆', 'china', 'mainland', '回国节点', '回国专线', '回国线路', '回国加速', '回国服务', '直连国内', '国内直连', '中转国内', '落地国内', '北京', '上海', '广州', '深圳', 'beijing', 'shanghai', 'guangzhou', 'shenzhen', '成都', '重庆', '杭州', '南京', '武汉', '天津', '苏州', '西安', '长沙', 'chengdu', 'chongqing', 'hangzhou', 'nanjing', 'wuhan', 'tianjin', 'suzhou', 'xian', 'changsha', '沈阳', '青岛', '郑州', '大连', '东莞', '宁波', '厦门', '济南', '无锡', '合肥', '昆明', '福州', '哈尔滨', '佛山', '长春', '石家庄', '太原', '南宁', '贵阳', '乌鲁木齐', '兰州', '海口', '银川', '西宁', '拉萨', '呼和浩特', '电信', '联通', '移动', '铁通', 'chinatelecom', 'chinaunicom', 'chinamobile', 'pek', 'pkx', 'pvg', 'szx', 'ctu', 'ckg', 'hgh', 'nkg', 'wuh', 'tsn', 'syx', 'xiy', 'csx', 'kmg', 'hak', 'dlc', 'tao', 'she', 'hrb', 'cgo'], iso: ['CN'] },
  { id: 'JP', kw: ['日本', '东京', '大阪', '横滨', '名古屋', '福冈', '札幌', '京都', '神户', '千叶', '埼玉', '仙台', '广岛', '冲绳', '那霸', 'japan', 'tokyo', 'osaka', 'yokohama', 'nagoya', 'fukuoka', 'sapporo', 'kyoto', 'kobe', 'chiba', 'sendai', 'hiroshima', 'okinawa', 'naha', 'nrt', 'hnd', 'kix', 'ngo', 'fuk', 'cts', 'oka'], iso: ['JP'] },
  { id: 'KR', kw: ['韩国', '首尔', '釜山', '仁川', '大田', '大邱', '光州', '济州', 'korea', 'seoul', 'busan', 'incheon', 'daejeon', 'daegu', 'gwangju', 'jeju', 'icn', 'gmp', 'pus'], iso: ['KR'] },
  { id: 'SG', kw: ['新加坡', 'singapore', 'sin'], iso: ['SG'] },
...
const BIZ = {
  ...
  EMAIL: '📧 邮件服务', IM: '💬 即时通讯', SOCIAL: '📱 社交媒体',
  WORK: '🧑‍💼 会议协作', CNMEDIA: '📺 国内流媒体', STREAM_SEA: '📺 东南亚流媒体',
  STREAM_US: '🇺🇸 美国流媒体', STREAM_HK: '🇭🇰 香港流媒体', STREAM_TW: '🇹🇼 台湾流媒体',
  STREAM_JP: '🇯🇵 日韩流媒体', STREAM_EU: '🇪🇺 欧洲流媒体',
  GAME_CN: '🕹️ 国内游戏', GAME_INTL: '🎮 国外游戏', SEARCH: '🔍 搜索引擎',
  DEV: '📟 开发者服务', MS: 'Ⓜ️ 微软服务', APPLE: '🍎 苹果服务',
  DOWNLOAD: '📥 下载更新', CLOUD_CDN: '☁️ 云与CDN', TRACKER: '🛰️ BT/PT Tracker',
  CN_SITE: '🏠 国内网站',
...
function injectBusinessGroups(config) {
  var groups = [
    { name: BIZ.AI, type: 'select', proxies: [SMART.US].concat(STANDARD_PROXIES.filter(function(p) { return p !== SMART.US })) },
...
function injectRules(config) {
  config.rules = [
    ...
    `DOMAIN-SUFFIX,youtube.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,youtu.be,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,googlevideo.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,ytimg.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,ggpht.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,youtube-nocookie.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,youtubekids.com,${BIZ.STREAM_US}`,
    ...
    // ════════════════════════════════════════════════════════════════
    // ── Google 邮件 ──
    `DOMAIN-SUFFIX,gmail.com,${BIZ.EMAIL}`,
    `DOMAIN-SUFFIX,googlemail.com,${BIZ.EMAIL}`,
    `DOMAIN,mail.google.com,${BIZ.EMAIL}`,
    `DOMAIN,inbox.google.com,${BIZ.EMAIL}`,
    // ── Google 即时通讯 ──
    ...
    `RULE-SET,googlesearch,${BIZ.SEARCH}`,
    `RULE-SET,googledrive,${BIZ.SEARCH}`,
    `RULE-SET,googleearth,${BIZ.SEARCH}`,
    `RULE-SET,google,${BIZ.SEARCH}`,
    `RULE-SET,google-ip,${BIZ.SEARCH},no-resolve`,
    // ════════════════════════════════════════════════════════════════
    ...
    `DOMAIN-SUFFIX,outlook.com,${BIZ.EMAIL}`,
    `DOMAIN-SUFFIX,outlook.live.com,${BIZ.EMAIL}`,
    `DOMAIN-SUFFIX,hotmail.com,${BIZ.EMAIL}`,
    `DOMAIN,mail.live.com,${BIZ.EMAIL}`,
    `DOMAIN,outlook.office365.com,${BIZ.EMAIL}`,
    `DOMAIN,outlook.office.com,${BIZ.EMAIL}`,
    `DOMAIN,mail.yahoo.com,${BIZ.EMAIL}`,
    `DOMAIN-SUFFIX,ymail.com,${BIZ.EMAIL}`,
    `DOMAIN-SUFFIX,protonmail.com,${BIZ.EMAIL}`,
    `DOMAIN-SUFFIX,proton.me,${BIZ.EMAIL}`,
    `DOMAIN-SUFFIX,pm.me,${BIZ.EMAIL}`,
    `DOMAIN-SUFFIX,tutanota.com,${BIZ.EMAIL}`,
    `DOMAIN-SUFFIX,tuta.com,${BIZ.EMAIL}`,
    // v5.1.3 FIX#7: Zoho 宽域名收窄为邮件专用子域名（防止吞掉 RULE-SET,zoho 会议协作规则）
    `DOMAIN,mail.zoho.com,${BIZ.EMAIL}`,
    `DOMAIN,mail.zoho.eu,${BIZ.EMAIL}`,
    `DOMAIN,mail.zoho.in,${BIZ.EMAIL}`,
    `DOMAIN,mail.zoho.com.au,${BIZ.EMAIL}`,
    `DOMAIN,mail.zoho.jp,${BIZ.EMAIL}`,
    `DOMAIN,mail.me.com,${BIZ.EMAIL}`,
    `DOMAIN-SUFFIX,fastmail.com,${BIZ.EMAIL}`,
    `DOMAIN-SUFFIX,fastmail.fm,${BIZ.EMAIL}`,
    `RULE-SET,mail,${BIZ.EMAIL}`,
    `RULE-SET,mailru,${BIZ.EMAIL}`,
    `RULE-SET,protonmail,${BIZ.EMAIL}`,
    `RULE-SET,spark,${BIZ.EMAIL}`,
    ...
    `RULE-SET,viu,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,wetv.vip,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,wetvinfo.com,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,iq.com,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,vidio.com,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,vidio.static6.com,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,rctiplus.com,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,visionplus.id,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,genflix.co.id,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,goplay.co.id,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,maxstream.tv,${BIZ.STREAM_SEA}`,
    `RULE-SET,biliintl,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,viki.com,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,viki.io,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,iflix.com,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,catchplay.com,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,mewatch.sg,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,trueid.net,${BIZ.STREAM_SEA}`,
    `DOMAIN-SUFFIX,dimsum.my,${BIZ.STREAM_SEA}`,
    `RULE-SET,asianmedia,${BIZ.STREAM_SEA}`,
    `RULE-SET,iqiyiintl,${BIZ.STREAM_SEA}`,
    `RULE-SET,joox,${BIZ.STREAM_SEA}`,
    `RULE-SET,mewatch,${BIZ.STREAM_SEA}`,
    `RULE-SET,viki,${BIZ.STREAM_SEA}`,
    `RULE-SET,wetv,${BIZ.STREAM_SEA}`,
    `RULE-SET,zee,${BIZ.STREAM_SEA}`,
    // v5.1.3 FIX#8: acc-kwai（Kwai国际版）移入东南亚流媒体（巴西/印尼主战场需代理）
    `RULE-SET,acc-kwai,${BIZ.STREAM_SEA}`,
    `RULE-SET,youtube,${BIZ.STREAM_US}`,
    `RULE-SET,netflix,${BIZ.STREAM_US}`,
    `RULE-SET,netflix-ip,${BIZ.STREAM_US},no-resolve`,
    `RULE-SET,spotify,${BIZ.STREAM_US}`,
    `RULE-SET,disney,${BIZ.STREAM_US}`,
    `RULE-SET,hbo,${BIZ.STREAM_US}`,
    `RULE-SET,primevideo,${BIZ.STREAM_US}`,
    `RULE-SET,hulu,${BIZ.STREAM_US}`,
    `RULE-SET,paramount,${BIZ.STREAM_US}`,
    `RULE-SET,peacock,${BIZ.STREAM_US}`,
    `RULE-SET,twitch,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,amazonaws.com,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,awsstatic.com,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,aws.amazon.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,elasticbeanstalk.com,${BIZ.DEV}`,
    `RULE-SET,amazon,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,crunchyroll.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,vrv.co,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,soundcloud.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,sndcdn.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,pandora.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,pluto.tv,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,tubi.tv,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,fubo.tv,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,discoveryplus.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,max.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,appletv.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,deezer.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,tidal.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,vimeo.com,${BIZ.STREAM_US}`,
    `DOMAIN-SUFFIX,dailymotion.com,${BIZ.STREAM_US}`,
    `RULE-SET,cbs,${BIZ.STREAM_US}`,
    `RULE-SET,nbc,${BIZ.STREAM_US}`,
    `RULE-SET,pbs,${BIZ.STREAM_US}`,
    `RULE-SET,attwatchtv,${BIZ.STREAM_US}`,
    `RULE-SET,fox,${BIZ.STREAM_US}`,
    `RULE-SET,fubotv,${BIZ.STREAM_US}`,
    `RULE-SET,sling,${BIZ.STREAM_US}`,
    `RULE-SET,soundcloud,${BIZ.STREAM_US}`,
    `RULE-SET,pandora,${BIZ.STREAM_US}`,
    `RULE-SET,pandoratv,${BIZ.STREAM_US}`,
    `RULE-SET,tidal,${BIZ.STREAM_US}`,
    `RULE-SET,vimeo,${BIZ.STREAM_US}`,
    `RULE-SET,dailymotion,${BIZ.STREAM_US}`,
    `RULE-SET,deezer,${BIZ.STREAM_US}`,
    `RULE-SET,discoveryplus,${BIZ.STREAM_US}`,
    `RULE-SET,overcast,${BIZ.STREAM_US}`,
    `RULE-SET,americasvoice,${BIZ.STREAM_US}`,
    `RULE-SET,cake,${BIZ.STREAM_US}`,
    `RULE-SET,dood,${BIZ.STREAM_US}`,
    // v5.1.3 FIX#9: ehgallery 从 STREAM_US 移到 INTL_SITE（非流媒体，全球节点更灵活）
    // RULE-SET,ehgallery 已移至国外网站区块
    `RULE-SET,lastfm,${BIZ.STREAM_US}`,
    `RULE-SET,emby,${BIZ.STREAM_US}`,
    // v5.1: szkane Netflix IP 段补充
    `RULE-SET,szkane-netflixip,${BIZ.STREAM_US},no-resolve`,
    ...
    `DOMAIN-SUFFIX,hulu.jp,${BIZ.STREAM_JP}`,
    `DOMAIN-SUFFIX,happyon.jp,${BIZ.STREAM_JP}`,
    ...
    `RULE-SET,bing,${BIZ.SEARCH}`,
    `DOMAIN-SUFFIX,yahoo.com,${BIZ.SEARCH}`,
    `DOMAIN-SUFFIX,yahoo.co.jp,${BIZ.SEARCH}`,
    `DOMAIN-SUFFIX,duckduckgo.com,${BIZ.SEARCH}`,
    `DOMAIN-SUFFIX,ddg.co,${BIZ.SEARCH}`,
    `DOMAIN-SUFFIX,brave.com,${BIZ.SEARCH}`,
    `DOMAIN-SUFFIX,yandex.com,${BIZ.SEARCH}`,
    `DOMAIN-SUFFIX,yandex.ru,${BIZ.SEARCH}`,
    `DOMAIN-SUFFIX,ecosia.org,${BIZ.SEARCH}`,
    `DOMAIN-SUFFIX,startpage.com,${BIZ.SEARCH}`,
    `DOMAIN-SUFFIX,you.com,${BIZ.SEARCH}`,
    `DOMAIN-SUFFIX,search.naver.com,${BIZ.SEARCH}`,
    ...
    `RULE-SET,scholar,${BIZ.SEARCH}`,
    `RULE-SET,yandex,${BIZ.SEARCH}`,
    `RULE-SET,github,${BIZ.DEV}`,
    ...
    `RULE-SET,docker,${BIZ.DEV}`,
    `RULE-SET,gitlab,${BIZ.DEV}`,
    `GEOSITE,category-dev,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,npmjs.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,npmjs.org,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,yarnpkg.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,pypi.org,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,pythonhosted.org,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,crates.io,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,rubygems.org,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,packagist.org,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,maven.org,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,nuget.org,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,cocoapods.org,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,stackoverflow.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,stackexchange.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,sstatic.net,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,vercel.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,vercel.app,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,netlify.app,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,netlify.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,pages.dev,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,workers.dev,${BIZ.DEV}`,
    ...
    `DOMAIN,dash.cloudflare.com,${BIZ.DEV}`,
    `DOMAIN,api.cloudflare.com,${BIZ.DEV}`,
    `DOMAIN,developers.cloudflare.com,${BIZ.DEV}`,
    `DOMAIN,www.cloudflare.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,heroku.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,herokuapp.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,fly.io,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,railway.app,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,render.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,supabase.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,supabase.co,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,planetscale.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,neon.tech,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,digitalocean.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,vultr.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,linode.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,sentry.io,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,datadog.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,grafana.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,postman.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,jetbrains.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,hashicorp.com,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,terraform.io,${BIZ.DEV}`,
    `DOMAIN-SUFFIX,vagrantup.com,${BIZ.DEV}`,
    `RULE-SET,developer,${BIZ.DEV}`,
    `RULE-SET,python,${BIZ.DEV}`,
    `RULE-SET,gitbook,${BIZ.DEV}`,
    `RULE-SET,jfrog,${BIZ.DEV}`,
    `RULE-SET,sublimetext,${BIZ.DEV}`,
    `RULE-SET,wordpress,${BIZ.DEV}`,
    `RULE-SET,wix,${BIZ.DEV}`,
    `RULE-SET,cisco,${BIZ.DEV}`,
    `RULE-SET,ibm,${BIZ.DEV}`,
    `RULE-SET,oracle,${BIZ.DEV}`,
    `RULE-SET,unity,${BIZ.DEV}`,
    // v5.1: szkane Developer（Docker镜像/模型文件下载）
    `RULE-SET,szkane-developer,${BIZ.DEV}`,
    ...
    `RULE-SET,cloudflare-ip,${BIZ.CLOUD_CDN},no-resolve`,
    `RULE-SET,cloudfront-ip,${BIZ.CLOUD_CDN},no-resolve`,
    `RULE-SET,fastly-ip,${BIZ.CLOUD_CDN},no-resolve`,
    `DOMAIN-SUFFIX,akamai.net,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,akamaized.net,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,akamaihd.net,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,akamaiedge.net,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,akamaitechnologies.com,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,edgekey.net,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,edgesuite.net,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,cloudfront.net,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,fastly.net,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,fastlylb.net,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,kxcdn.com,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,stackpathdns.com,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,stackpathcdn.com,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,b-cdn.net,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,bunny.net,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,bunnycdn.com,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,cdn77.org,${BIZ.CLOUD_CDN}`,
    ...
    `DOMAIN-SUFFIX,ucoz.ru,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,ucoz.net,${BIZ.CLOUD_CDN}`,
    `RULE-SET,cloudflare,${BIZ.CLOUD_CDN}`,
    `RULE-SET,akamai,${BIZ.CLOUD_CDN}`,
    // v5.1.2 FIX#6: RULE-SET,dns 已删除（DNS自然分流，不锁死代理组）
    `RULE-SET,digicert,${BIZ.CLOUD_CDN}`,
    `RULE-SET,globalsign,${BIZ.CLOUD_CDN}`,
    `RULE-SET,sectigo,${BIZ.CLOUD_CDN}`,
    `RULE-SET,brightcove,${BIZ.CLOUD_CDN}`,
    `RULE-SET,jwplayer,${BIZ.CLOUD_CDN}`,
    // v5.1: Accademia CDN 补充
    `RULE-SET,acc-fastly,${BIZ.CLOUD_CDN}`,
    // v5.1.2 FIX#6: RULE-SET,acc-globaldns 已删除（DNS自然分流）
    `DOMAIN-SUFFIX,letsencrypt.org,${BIZ.CLOUD_CDN}`,
    `DOMAIN-SUFFIX,lencr.org,${BIZ.CLOUD_CDN}`,
    ...
    `GEOIP,cloudflare,${BIZ.CLOUD_CDN},no-resolve`,
    `GEOIP,telegram,${BIZ.IM},no-resolve`,
    `GEOIP,netflix,${BIZ.STREAM_US},no-resolve`,
    `GEOIP,facebook,${BIZ.SOCIAL},no-resolve`,
    `GEOIP,twitter,${BIZ.SOCIAL},no-resolve`,
    `GEOIP,google,${BIZ.SEARCH},no-resolve`,
...
function cleanupSubscription(config) {
  const groupsToDelete = ['负载均衡', '自动选择', '手动选择', '节点选择']
  const deletedNames = new Set()
  config['proxy-groups'] = config['proxy-groups'].filter(g => {
    if (!g || !g.name) return true
    for (const kw of groupsToDelete) { if (g.name.includes(kw)) { deletedNames.add(g.name); return false } }
    return true
  })
  if (deletedNames.size > 0) {
    config['proxy-groups'].forEach(group => {
      if (!group || !Array.isArray(group.proxies)) return
      group.proxies = group.proxies.filter(p => !deletedNames.has(p))
    })
  }
  config.rules = []
